...
class TalkEntryScreen extends StatefulWidget {
  const TalkEntryScreen({super.key});
  @override
  State<TalkEntryScreen> createState() => _TalkEntryScreenState();
}
...
class _TalkEntryScreenState extends State<TalkEntryScreen> {
  ...
  Future<void> _startNow() async {
    ...
    final sessionId =
        await LiveTalkService.instance.createSession(answers: answers);
    if (!mounted) return;
    Navigator.of(context).push(MaterialPageRoute(
        builder: (_) => LiveTalkCallScreen(sessionId: sessionId)));
  ...
  Future<void> _schedule() async {
    ...
    ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('Scheduled! We’ll remind you.')));
    ...
    return Scaffold(
      ...
      body: Padding(
        ...
        child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          Text('Talk through your interview',
              style: Theme.of(context).textTheme.titleLarge),
          const SizedBox(height: 8),
          const Text(
              'Choose start now or schedule a time. You can switch back to text any time.'),
